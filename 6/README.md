...
- Читает DSN из переменной окружения DB_DSN
...
Параметры подключения задаются через переменную окружения DB_DSN в формате:
```
